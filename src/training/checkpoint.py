...
class CheckpointMetadata:
    def __init__(self, model_kwargs, optimizer_kwargs, is_fsdp: bool, num_accelerators: int):
...
class CheckpointManager:
    ...
    def ready(self):
        return self.model is not None and self.optimizer is not None and self.accelerator is not None and self.metadata is not None
    ...
    def save(self, save_directory):
        if not self.ready():
            raise RuntimeError("CheckpointManager is not ready. Please prepare it with model, optimizer, accelerator, and metadata.")
    ...
    def merge_model_shards(self, model_shard_dir, output_dir):
        if not os.path.exists(model_shard_dir):
            raise FileNotFoundError(f"Model shard directory {model_shard_dir} does not exist.")
        if os.path.exists(output_dir) and os.listdir(output_dir):
            if not self.metadata.is_fsdp:
                raise FileExistsError(f"Output directory {output_dir} already exists and is not empty. Please choose a different directory.")
            else:
    ...
    def load(self, load_directory, error_if_not_exists: bool = True):
        ...
        if not os.path.exists(load_directory):
            if error_if_not_exists:
                raise FileNotFoundError(f"Checkpoint directory {load_directory} does not exist.")
            else:
                self.accelerator.print(f"Checkpoint directory {load_directory} does not exist. Skipping load.")
        ...
        if not os.path.exists(os.path.join(load_directory, "metadata.json")):
            if error_if_not_exists:
                ...
                self.accelerator.print(f"Metadata file not found in {load_directory}. Skipping load.")
                self.model, self.optimizer = self.accelerator.prepare(self.model, self.optimizer)
                return
        """
        ...
        if not is_fsdp and not self.metadata.is_fsdp:
            ...
            # Case 3
            self.merge_model_shards(load_directory + "/pytorch_model_fsdp_0", load_directory + "_merged")
            self.model.load_state_dict(torch.load(os.path.join(load_directory + "_merged", "pytorch_model.bin")))
            self.model, self.optimizer = self.accelerator.prepare(self.model, self.optimizer)
            ...
            raise RuntimeError(f"Inconsistent checkpoint metadata. Checkpoint metadata FSDP is {is_fsdp}, current FSDP is {self.metadata.is_fsdp}. "
