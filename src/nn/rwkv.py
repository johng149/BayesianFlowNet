...
from torch.nn import GELU, Linear, Module, RMSNorm, Sequential
from torch.nn import functional as F

from src.common.sequence import doc_ids_to_cu_seqlen

## Slight Custom Tweak to RWKV7
...
class RWKV7Attention(nn.Module):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        if decay_low_rank_dim is None:
            decay_low_rank_dim = max(
                32, int(round((2.5 * (hidden_size**0.5)) * factor / 32) * 32)
            )
        ...
        if v_low_rank_dim is None:
            v_low_rank_dim = max(
                32, int(round((1.7 * (hidden_size**0.5)) * factor / 32) * 32)
            )
        ...
        self.a_lora = LoRA(
            hidden_size, self.key_dim, low_rank_dim=a_low_rank_dim, activation=None
        )
        self.g_lora = LoRA(
            hidden_size,
            self.value_dim,
        ...
        try:
            ...
            _init_weights = True
        if _init_weights:
            self.apply(self._initialize_weights)  # pyright: ignore[reportArgumentType]
        for name, module in self.named_modules():
            module._in_rwkv_module = True  # pyright: ignore[reportArgumentType]
    ...
    def _initialize_weights(self, module: nn.Module):
        ...
        if isinstance(module, RWKV7Attention) and self.layer_idx is not None:
            ...
            ratio_1_to_almost0 = 1.0 - (
                self.layer_idx / self.num_hidden_layers
            )  # 1 to ~0
            ...
            for n in range(self.hidden_size):
                ...
                www[n] = -6 + 6 * (n / (self.hidden_size - 1)) ** (
                    1 + 1 * ratio_0_to_1**0.3
                )
                ddd[0, 0, n] = n / self.hidden_size

            # Initialize x_* parameters directly
            self.x_r.data = (1.0 - torch.pow(ddd, 0.2 * ratio_1_to_almost0)).to(
                self.x_r.dtype
            )
            self.x_w.data = (1.0 - torch.pow(ddd, 0.9 * ratio_1_to_almost0)).to(
                self.x_w.dtype
            )
            self.x_k.data = (1.0 - torch.pow(ddd, 0.7 * ratio_1_to_almost0)).to(
                self.x_k.dtype
            )
            self.x_v.data = (1.0 - torch.pow(ddd, 0.7 * ratio_1_to_almost0)).to(
                self.x_v.dtype
            )
            self.x_a.data = (1.0 - torch.pow(ddd, 0.9 * ratio_1_to_almost0)).to(
                self.x_a.dtype
            )
            self.x_g.data = (1.0 - torch.pow(ddd, 0.2 * ratio_1_to_almost0)).to(
                self.x_g.dtype
            )
            ...
            self.k_k.data.copy_(
                (
                    torch.zeros(self.hidden_size, device=self.k_k.device)
                    + 0.71
                    - linear * 0.1
                ).to(self.k_k.dtype)
            )
        ...
        module._is_hf_initialized = True  # pyright: ignore[reportArgumentType]
    ...
    def forward(
        ...
        **kwargs,
    ) -> Tuple[Tensor, Optional[Tensor], Optional[Cache], Tensor]:
        if v_first is None:
        ...
        if self.layer_idx == 0:
            ...
            xv_lora_weights = self.v_lora(xv).sigmoid()
            v = torch.lerp(v, v_first.to(xv_lora_weights.dtype), xv_lora_weights)
        ...
        assert (
            isinstance(r, Tensor)
            and isinstance(w, Tensor)
            and isinstance(k, Tensor)
        ...
        if self.training or seq_len >= 64:
            ...
            o, recurrent_state = chunk_rwkv7(
                ...
                b=kk * a,
                scale=1.0,
                initial_state=recurrent_state,  # pyright: ignore[reportArgumentType]
        ...
        if past_key_values is not None and self.layer_idx is not None:
            past_key_values.update(
                recurrent_state=recurrent_state,  # pyright: ignore[reportArgumentType]
        ...
        o = self.o_proj(o)
        assert isinstance(v_first, Tensor)
...
class RWKV(Module):
    def __init__(self, hidden_dim: int, num_heads: int):
        ...
        self.mlp_out = Sequential(
            Linear(hidden_dim, hidden_dim * 3),
            GELU(),
            Linear(hidden_dim * 3, hidden_dim),
        )

    def forward(self, x, seq_idx) -> Tensor:
        assert seq_idx.ndim == 2, "seq_idx must be of shape (batch_size, seq_len)"
        assert (
            x.shape[0] == seq_idx.shape[0] and x.shape[1] == seq_idx.shape[1]
        ), "x and seq_idx must have the same batch_size and seq_len"
        cu_seqlen = doc_ids_to_cu_seqlen(seq_idx)
        v_first = torch.zeros_like(
            x,
            dtype=self.attention.v_lora.lora[
                0
            ].weight.dtype,  # pyright: ignore[reportArgumentType]
        )

        residual = x
        x = self.attn_norm(x)
        x, attentions, past_kv, v_first = self.attention(
            hidden_states=x, cu_seqlen=cu_seqlen, v_first=v_first
        )
        x = x + residual
        residual = x
        x = self.ffn_norm(x)
